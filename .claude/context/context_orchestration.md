...
## Automatic Turn Initialization

**Pre-execution steps (1-4) are now handled automatically** by the `turn-init.sh` hook on every `UserPromptSubmit`. The hook:

1. Resolves the next TURN_ID from `turns_index.csv`
2. Creates the turn directory: `./ai/agentic-pipeline/turns/turn-${TURN_ID}/`
3. Writes `session_context.md` with the user prompt and metadata
4. Records TURN_START_TIME

**You (Claude) are responsible for post-execution steps (6-10)** after completing any response that modifies files.
